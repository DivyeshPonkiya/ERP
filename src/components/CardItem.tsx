...
import {DeleteSvg, DetailTitleSvg} from '../assets/Images/svg';
...
const CardItem = ({item, onPress, deleteModal}: CardProps) => {
  ...
  const menuItems: MenuItem[] = [
    ...
    },
    {
      ...
      onPress: () => {
        navigate(NAVIGATION.AddSales, item);
      },
    },
    {
    ...
    <Pressable style={styles.card} onPress={onPress}>
        ...
        <View
          ...
          <View style={styles.imageWrapper}>
            {item?.product?.cover_image_url ? (
              <FastImageView
                source={item?.product?.cover_image_url}
                resizeMode="contain"
                style={styles.image}
              />
            ) : (
      ...
      <View style={styles.bottomRow}>
        ...
        <View style={styles.infoBox}>
          ...
          />
          <View style={{marginHorizontal: hs(8)}}>
            <Text style={styles.label}>{strings.customer}</Text>
            <Text style={styles.value} numberOfLines={1}>
              {CFL(item?.customer?.name)}
            </Text>
          </View>
        </View>
